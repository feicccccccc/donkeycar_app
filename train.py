...
from donkeycar.parts.keras import KerasLinear, KerasIMU, KerasIMU2, \
    ...
    KerasRNN_LSTM, KerasLatent, KerasLocalizer, Keras_IMU_LSTM_Categorical, Keras_IMU_LSTM_Many2Many_Categorical, Keras_IMU_LSTM_Linear
...
def collate_records(records, gen_records, opts):
    ...
    for record_path in records:
        ...
        try:
            ...
            accl_x1 = float(json_data['imu1/acl_x'])
            accl_y1 = float(json_data['imu1/acl_y'])
            accl_z1 = float(json_data['imu1/acl_z'])

            gyro_x1 = float(json_data['imu2/gyr_x'])
            gyro_y1 = float(json_data['imu2/gyr_y'])
            gyro_z1 = float(json_data['imu2/gyr_z'])

            accl_x2 = float(json_data['imu2/acl_x'])
            accl_y2 = float(json_data['imu2/acl_y'])
            accl_z2 = float(json_data['imu2/acl_z'])

            gyro_x2 = float(json_data['imu2/gyr_x'])
            gyro_y2 = float(json_data['imu2/gyr_y'])
            gyro_z2 = float(json_data['imu2/gyr_z'])

            sample['imu_array'] = np.array([accl_x1, accl_y1, accl_z1, gyro_x1, gyro_y1, gyro_z1,
                                            accl_x2, accl_y2, accl_z2, gyro_x2, gyro_y2, gyro_z2])
        except:
            pass
...
def train(cfg, tub_names, model_name, transfer_model, model_type, continuous, aug):
    ...
    if cfg.OPTIMIZER:
        ...
        if isinstance(kl, Keras_IMU_LSTM_Categorical):
            pass
        if isinstance(kl, Keras_IMU_LSTM_Many2Many_Categorical):
            pass
        if isinstance(kl, Keras_IMU_LSTM_Linear):
            pass
            ...
            for offset in range(0, num_records, batch_size):
                ...
                for seq in batch_data:
                    ...
                    if cfg.model_type == 'rnn_imu':
                        ...
                        label_vec2 = dk.utils.linear_bin(throttle, N=31, offset=0, R=cfg.MODEL_CATEGORICAL_MAX_THROTTLE_RANGE)
                ...
                if cfg.model_type == 'look_ahead':
                    ...
                    X1 = np.array(b_inputs_img).reshape(batch_size, cfg.SEQUENCE_LENGTH, cfg.TARGET_H, cfg.TARGET_W,
                                                        cfg.TARGET_D)
                    X2 = np.array(b_vec_in).reshape(batch_size, cfg.SEQUENCE_LENGTH, 12)
                    X3 = np.array(b_steer_in).reshape(batch_size, cfg.SEQUENCE_LENGTH, 31)
                    X4 = np.array(b_throttle_in).reshape(batch_size, cfg.SEQUENCE_LENGTH, 31)
...
def multi_train(cfg, tub, model, transfer, model_type, continuous, aug):
    ...
    '''
    train_fn = train
    if model_type in ("rnn", '3d', 'look_ahead', 'rnn_imu', 'rnn_imu_linear', 'rnn_imu_many2many', 'rnn_imu_many2many_imupred', 'test'):
